...
if __name__ == "__main__":
  ...
  dl_pkt_interval = dl_pkt_interval[dl_pkt_interval < 0.075] * 1000 # in ms
  ul_pkt_interval = ul_pkt_interval[ul_pkt_interval < 0.2] * 1000 # in ms

  dl_count, dl_bins_count = np.histogram(dl_pkt_interval, bins=1000) 
  dl_pdf = dl_count / sum(dl_count) 
  dl_cdf = np.cumsum(dl_pdf) 

  ul_count, ul_bins_count = np.histogram(ul_pkt_interval, bins=1000) 
